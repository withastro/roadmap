...
- For dynamic pages, they will be served in the `content-security-policyt` header of the `Response` type.

A [new adapter feature](https://docs.astro.build/en/reference/adapter-reference/#adapter-features) will be provided, called `hostedCsp`. For adapters that support this feature, Astro **won't** serve the hashes for *static pages*`.

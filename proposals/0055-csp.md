...
- Reference Issues: 
- Implementation PR: https://github.com/withastro/astro/pull/13802
- Stage 2 Issue: https://github.com/withastro/roadmap/issues/1149
- Stage 3 PR: https://github.com/withastro/roadmap/pull/1168
...
- Automatic support for third-party scripts dynamically imported by users e.g. `document.createElement("script")`, `import("https://path.to/script.js")`;
- Automatic support for third-party styles dynamically imported by users.
- Performance. Users and authors are willing to overlook possible performance drawbacks in order to increase security.
- Support the [dev server](#dev-server)
...
#### `Astro.csp.insertDirective(directive: CspDirective)`

Users can add an entire directive. Astro won't check if the directive is grammatically correct. Astro will remove deduplicate resources.

```js
Astro.csp.insertDirective(`image-src ${Astro.site}`)
```
Result
```html
<meta 
...
#### `Astro.csp.insertStyleHash(string)`

```js
Astro.csp.insertStyleHash("sha256-qewpiorfuvng3")
```
Result
```html
<meta 
...
#### `Astro.csp.setStyleResource(string)`

Astro will deduplicate and remove duplicated resources.

```js
Astro.csp.setStyleResource("https://styles.cdn.example.com")
```
Result
```html
<meta 
...
#### `Astro.csp.insertScriptHash(string)`

```js
Astro.csp.insertScriptHash("sha256-qewpiorfuvng3")
```
Result
```html
<meta 
...
#### `Astro.csp.setScriptResource(string)`

Astro will deduplicate and remove duplicated resources.

```js
Astro.csp.setScriptResource("https://scripts.cdn.example.com")
```
Result
```html
<meta 
...
- The new Astro feature will be shipped with the `experimental` prefix: `experimentalStaticHeaders` in a patch release.
- The new field of the hook `astro:build:generated` will be shipped with the `experimental` prefix: `experimentalRouteToHeaders` in a patch release.
